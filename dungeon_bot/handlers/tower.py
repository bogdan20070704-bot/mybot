...
async def handle_tower_leave(callback: CallbackQuery, user_id: int, tower: TowerRun):
    """Покинуть Башню"""
    try:
        exp_reward = tower.exp_gained // 2
        coin_reward = tower.coins_gained // 2

        if exp_reward > 0 or coin_reward > 0:
            await db.add_exp(user_id, exp_reward)
            await db.add_coins(user_id, coin_reward)
            rewards_text = f"\n💰 Получено (x0.5):\n⭐ +{exp_reward} опыта\n🪙 +{coin_reward} монет"
        else:
        ...
        try:
            await callback.answer("⚠️ Не удалось корректно завершить забег.", show_alert=True)
        except Exception:
...
async def complete_tower(callback: CallbackQuery, user_id: int, user_data: dict, tower: TowerRun, success: bool):
    ...
    try:
        ...
        try:
            await callback.message.answer(
                "⚠️ Внутренняя ошибка при завершении забега. Состояние сброшено, можно попробовать снова.",
                reply_markup=main_menu_keyboard(),
            )
        except Exception:
