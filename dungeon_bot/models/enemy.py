...
def get_random_mob() -> Enemy:
    """Получить случайного обычного моба"""
    import random
    mobs = [e for e in ENEMIES_DB.values() if e.enemy_type == 'mob']
    return random.choice(mobs)


def get_random_miniboss() -> Enemy:
    """Получить случайного мини-босса"""
    import random
    minibosses = [e for e in ENEMIES_DB.values() if e.enemy_type == 'miniboss']
    return random.choice(minibosses)


def get_random_boss() -> Enemy:
    """Получить случайного босса"""
    import random
    bosses = [e for e in ENEMIES_DB.values() if e.enemy_type == 'boss']
